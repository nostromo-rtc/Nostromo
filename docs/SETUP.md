...
3. Укажите локальный и внешний IP-адрес сервера в параметрах `MEDIASERVER_LOCAL_IP` и `MEDIASERVER_ANNOUNCED_IP` для работы медиасервера.
    > Если планируется работа в локальной сети, то в `MEDIASERVER_ANNOUNCED_IP` можно ничего не писать.

4. Укажите диапазон портов для медиасервера от `MEDIASERVER_RTC_MIN_PORT` до `MEDIASERVER_RTC_MAX_PORT`.
