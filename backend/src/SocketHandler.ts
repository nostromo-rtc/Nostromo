...
type RoomForUser = { id: RoomId, name: RoomInfo["name"]; };
...
export class SocketHandler
{
    ...
    constructor(server: https.Server, sessionMiddleware: RequestHandler, rooms: Map<RoomId, RoomInfo>)
    {
        ...
        this.io.of('/admin').on('connection', (socket: Socket) =>
        {
            ...
            else
            {
                ...
                socket.emit('roomList', roomList, rooms.size);

                socket.on('deleteRoom', (id: RoomId) => rooms.delete(id));

                socket.on('createRoom', (name: string, pass: string) =>
                {
                    rooms.set(String(rooms.size), { name: name, password: pass });
                });
            }
        });
        ...
        {
            if (socket.handshake.session!.activeRoomID)
            {
        ...
        this.io.of('/room').use((socket: Socket, next) =>
        {
            ...
            if (socket.handshake.session!.auth)
            {
                const activeRoomID: string | undefined = socket.handshake.session!.activeRoomID;
